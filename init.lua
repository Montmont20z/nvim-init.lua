...
require("lazy").setup({
  ...
  "nvim-tree/nvim-tree.lua",
	...
	require("nvim-tree").setup({
        ...
        vim.keymap.set("n", "<leader>ff", ":NvimTreeFindFile<CR>", { desc = "Reveal current file in tree" })
      end
    },
    { "kylechui/nvim-surround", version = "^3.0.0", config = function()
    ...
    },
    {
      ...
      config = function()
        ...
        -- helper: check if there are words before cursor (for tab behavior)
        ...
        cmp.setup({
          ...
          mapping = {
            ...
            -- Tab: next item / expand or jump snippet / trigger completion / fallback
        ...
        -- Optional: use cmp for `/` and `:` (cmdline) as well
        -- cmp.setup.cmdline("/", {
    ...
    { "rafamadriz/friendly-snippets" }, -- prebuild code snippet
})


-- make Space do nothing in normal/visual/operator-pending modes
-- prevents the cursor-moving behavior when you press Space alone
...
-- CTRL + Left Click → go to definition (works in normal mode when clicking)
vim.keymap.set('n', '<C-LeftMouse>', function()
  vim.lsp.buf.definition()
end, { noremap=true, silent=true, desc = "Go to definition (Ctrl+Click)" })
...
require('nvim-treesitter.configs').setup {
  ensure_installed = {
    "java","python","lua","bash","json","html","css","javascript","typescript","markdown"
  }, -- add languages you use
  highlight = {
    enable = true,              -- false will disable the whole extension
    additional_vim_regex_highlighting = false, -- set to true for some filetypes where TS is incomplete
  },
  indent = { enable = true },   -- TS-based indentation
  autotag = { enable = true },  -- auto rename/close HTML-like tags
  textobjects = {
    select = {
      enable = true,
      lookahead = true,
  ...
  rainbow = {
    enable = true,               -- requires p00f/nvim-ts-rainbow
    extended_mode = true,        -- highlight non-bracket delimiters (HTML, JSX)
    max_file_lines = nil,        -- disable for very large files if you want
  },
  playground = {
    enable = true,
    disable = {},
...
-- lazy.nvim plugin added earlier; then:
